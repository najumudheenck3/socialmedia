...
import { verifyAccount } from '../../api/user/AuthRequest'
...
const Verify = () => {
    ...
    const verifyEmail=async(e)=>{
        e.preventDefault();
        let verifyStatus=await verifyAccount(userId,token)
        console.log(verifyStatus.data);
        if(verifyStatus.data.success){
            toast.success(verifyStatus.data.message);
            navigate('/login')
        }else{
            toast.error(verifyStatus.data.message)
        }
    }
    
  return (
    <div class="flex items-center justify-center min-h-screen p-5 bg-specclr min-w-screen">
    <div class="max-w-xl p-8 text-center text-gray-800 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
        <h3 class="text-2xl">Thanks for signing up for quickshare.com!</h3>
        ...
        <div class="mt-4">
            <button onClick={verifyEmail} class="px-2 py-2 text-blue-200 bg-specclr rounded">Click to Verify Email</button>
